...
class WC_Ingpsp_AfterPay extends WC_Ingpsp_Gateway
{
    ...
    /**
     * WC_Ingpsp_AfterPay constructor.
     */
    public function __construct()
    {
        $this->id = 'ingpsp_afterpay';
        $this->icon = false;
        $this->has_fields = true;
        $this->method_title = __('ING PSP : AfterPay', WC_Ingpsp_Helper::DOMAIN);
        $this->method_description = __('ING PSP : AfterPay', WC_Ingpsp_Helper::DOMAIN);
    ...
    public function process_payment($order_id)
    {
        ...
        $ingOrder = $this->ing->createAfterPayOrder(
        ...
        if ($ingOrder->status()->isError()) {
            ...
            wc_add_notice(
                __('Unfortunately, we can not currently accept your purchase with AfterPay. Please choose another payment option to complete your order. We apologize for the inconvenience.', WC_Ingpsp_Helper::DOMAIN),
                'error'
            );
            return [
                'result' => 'failure',
        ...
        $order->update_status('on-hold', __('AfterPay payment request sent.', WC_Ingpsp_Helper::DOMAIN));
    ...
    public function handle_thankyou($order_id)
    {
        ...
        echo "<p><b>".__('Your payment using AfterPay is successful.', WC_Ingpsp_Helper::DOMAIN)."</b></p>";
    ...
    public function payment_fields()
    {
        ...
        woocommerce_form_field('toc', array(
            ...
            'class' => array('input-text'),
            'label' => sprintf(
                __("I accept <a href='%s' target='_blank'>Terms and Conditions</a>", WC_Ingpsp_Helper::DOMAIN),
